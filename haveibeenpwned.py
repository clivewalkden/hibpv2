...
parser = \
    argparse.ArgumentParser(description='Verify if email address has been pwned'
                            )
...
def checkAddress(email):
    ...
    if str(check.status_code) == '404':  # The address has not been breached.
        ...
        print WARNING \
            ...
            + check.headers['Retry-After'] + ' seconds' + ENDC
        print WARNING \
        ...
        print WARNING + '[!] Something went wrong while checking ' \
            + email + ENDC
        time.sleep(sleep)  # sleep so that we don't trigger the rate limit
        return True
